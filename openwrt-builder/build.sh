...
OPTION_TOOLCHAIN_ONLY=${OPTION_TOOLCHAIN_ONLY:-false}
# clean build !slow!
OPTION_CLEAN_BUILD=${OPTION_CLEAN_BUILD:-false}
# enable wireless
OPTION_ENABLE_WIRELESS=${OPTION_ENABLE_WIRELESS:-true}
# build thread
...
# disable wireless
if [ "$OPTION_ENABLE_WIRELESS" = "false" ]; then
...
if [ "$OPTION_TOOLCHAIN_ONLY" = "true" ]; then
	make -j$(nproc) toolchain/install
	exit 0
fi

# make build
make -j${OPTION_BUILD_THREAD} V=s
