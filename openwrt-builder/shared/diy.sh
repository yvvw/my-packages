...
# kernel patch
find "target/linux" -type f -path "target/linux/*/config-*" | while read -r config; do
	if ! grep -q 'CONFIG_FAT_DEFAULT_IOCHARSET="utf8"' "$config"; then
		sed -i '$a \
CONFIG_FAT_DEFAULT_IOCHARSET="utf8" \
CONFIG_CRYPTO_CHACHA20_NEON=y \
CONFIG_CRYPTO_CHACHA20POLY1305=y' "$config"
	fi
done
